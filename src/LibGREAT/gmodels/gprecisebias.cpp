...
bool t_gprecisebias::cmb_equ(t_gtime &epoch, t_gallpar &params, t_gsatdata &obsdata, t_gobs &gobs,
...
double t_gprecisebias::tropoDelay(t_gtime &epoch, string &rec, t_gallpar &param, t_gtriple site_ell,
                                  t_gsatdata &satdata)
{
...
double t_gprecisebias::ionoDelay(t_gtime &epoch, t_gallpar &param, t_gsatdata &satdata, IONMODEL &ion_model,
                                 GOBSBAND &band_1, t_gobs &gobs)
{
    ...
    if (gobs.band() == BAND || gobs.band() == BAND_A || gobs.band() == BAND_B || gobs.band() == BAND_C ||
        gobs.band() == BAND_D)
    {
        return 0.0;
    }
...
double t_gprecisebias::PCV(bool corrt_sat, bool corrt_rec, t_gtime &epoch, t_gtime &crt_sat_epo, t_gtriple &trs_rec_crd,
                           t_gsatdata &satdata, t_gobs &gobs)
{
...
Matrix t_gprecisebias::_RotMatrix_Ant(t_gsatdata &obsdata, const t_gtime &receive_epoch, const t_gtime &transmit_epoch,
                                      shared_ptr<t_gobj> obj, bool isCRS)
{
    ...
    else if (type == t_gdata::REC)
    {
        ...
        sinLam = sin(ell[1]);
        cosLam = cos(ell[1]);
        rotmatrix << -sinPhi * cosLam << -sinLam << +cosPhi * cosLam << -sinPhi * sinLam << +cosLam << +cosPhi * sinLam
                  << +cosPhi << 0.0 << +sinPhi;
        t = receive_epoch;
    }
    else
...
bool t_gprecisebias::_get_crs_sat_vel(const t_gtime &sat_epoch, const string &sat, t_gallnav *nav,
                                      t_gtriple &crs_sat_vel)
{
    if (!nav)
    {
        if (_spdlog)
...
bool t_gprecisebias::_get_crs_sat_crd(const t_gtime &sat_epoch, const string &sat, t_gallnav *nav,
                                      t_gtriple &crs_sat_crd)
{
...
bool t_gprecisebias::_apply_rec_tides(const t_gtime &epoch, t_gtriple &rec)
{
    ...
    try
    {
        ...
        if (_opl)
        {
            t_gtideIERS *opl_ptr = dynamic_cast<t_gtideIERS *>(
                _tide.get()); // please note that in _tide. no pointer for atmloading is stored.
