...
void great::gviewer::SetFrames(const Frames &f)
{
    unique_lock<mutex> lock(m_mutex);
    mv_frames.clear();
...
void great::gviewer::SetPointCloud(const VPointCloud &pc)
{
    unique_lock<mutex> lock(m_mutex);
    mv_pointCloud.clear();
...
void great::gviewer::SetFrames(const vector<Frames> &vf)
{
    unique_lock<mutex> lock(m_mutex);
    mv_frames = vf;
}
...
void great::gviewer::SetPointCloud(const vector<VPointCloud> &vpc)
{
    unique_lock<mutex> lock(m_mutex);
    mv_pointCloud = vpc;
}
