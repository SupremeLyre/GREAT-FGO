...
/******************************************* t_gintegration ***********************************************/
great::t_gintegration::t_gintegration(string site, string site_base, t_gsetbase *gset,
                                      std::shared_ptr<spdlog::logger> spdlog, t_gallproc *allproc)
    : t_gsinskf(gset, spdlog, (/*site_base+*/ site)), t_gspp(site, gset, spdlog),
...
int great::t_gintegration::processBatchFB(const t_gtime &beg, const t_gtime &end, bool beg_end)
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_ERROR(_spdlog, string("gintegration:  ") + "init failed");
        return -1;
    }
    ...
    else
    {
        ...
        if (_gnss_crt < _ins_crt)
        {
            ...
            if (_sampling > 1)
                _gnss_crt.add_secs(int(_sampling * nEpo)); //  < 1Hz data
            else
                _gnss_crt.add_dsec(_sampling * nEpo); //  >=1Hz data
        }
    }

    std::cerr << endl
              << _site << ": Start MSF Processing:" << _ins_crt.str_ymdhms() << "  " << _end_time.str_ymdhms() << endl;
    ...
    while (time_loop)
    {
        ...
        {
            if (_ign_type == IGN_TYPE::TCI && !_initial_merge)
                _merge_init();
            ...
            if (_Meas_Type.size())
            {
                ...
                cerr << "\r" << _ins_crt.str_ymdhms("Processing Epoch: ") << " Meas = " << meas2str(*_Meas_Type.begin())
                     << fixed << setprecision(1) << setw(6) << percent << "%";
                if (_spdlog)
                    SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + _site + ": ( " + outinfo + ")" +
        ...
        {
            if (_sampling > 1)
                _gnss_crt.add_secs(int(_sampling)); // =<1Hz data
            else
                _gnss_crt.add_dsec(_sampling); //  >1Hz data
        }
    }
...
int great::t_gintegration::_gnss_feedback(const ColumnVector &dx)
{
    try
    {
        ...
        for (unsigned int iPar = 0; iPar < _param.parNumber(); iPar++)
        {
            if (_param[iPar].parType == par_type::TRP)
            {
                ...
                {
                    XYZ = _gallobj->obj(site)->crd_arp(_epoch);
                }
        ...
        for (unsigned int iPar = 0; iPar < _param.parNumber(); iPar++)
        {
            if (_param[iPar].parType == par_type::CRD_X || _param[iPar].parType == par_type::CRD_Y ||
                _param[iPar].parType == par_type::CRD_Z)
            {
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + "GNSS feedback failed!");
        //_reset();
        Flag = NO_MEAS;
        return -1;
    }
...
void great::t_gintegration::_feedback(MEAS_TYPE type, bool successed)
{
    if (type == MEAS_TYPE::GNSS_MEAS && _ign_type == TCI)
    {
        ...
        if (_amb_state)
            Xk = Columns2VectorXd(_filter->dx()).block(0, 0, nq, 1);
        this->feedback();
        if (_amb_state)
            this->_write();
        sins = sins_out;
    ...
    Pk = _global_variance;
    if (_ign_type == TCI)
        _Qx = Eigen2BaseMatrix(_global_variance);

    // GNSS feedback
    if (_ign_type == IGN_TYPE::TCI)
    {
        ColumnVector dx;
        dx.ReSize(global_size);
...
int great::t_gintegration::_init()
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_ERROR(_spdlog, string("gintegration:  ") + "IMU observation is not existing!!! ");
        return -1;
    }
...
{
    if ((_ign_type == IGN_TYPE::TCI || _ign_type == IGN_TYPE::VIS_TCI || _ign_type == IGN_TYPE::LIDAR_TCI ||
         _ign_type == IGN_TYPE::MULTIGN_TCI || _ign_type == IGN_TYPE::UWB_TCI))
    {
        try
        {
...
int great::t_gintegration::_processEpoch(const t_gtime &runEpoch)
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + "No receiver settings available!!!");
        return NO_MEAS;
    }
    ...
    do
    {
        ...
        if (_valid_ins_constraint())
        {
            ...
            t_gtriple Qxyz(sqrt(_Qx(crd_idx + 1, crd_idx + 1)), sqrt(_Qx(crd_idx + 2, crd_idx + 2)),
                           sqrt(_Qx(crd_idx + 3, crd_idx + 3)));
        ...
        {
            if (_spdlog)
                SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + "Not enough visible satellites!");
        ...
        if (_isBase)
        {
            ...
            dynamic_cast<t_gcombDD *>(&(*_base_model))
                ->set_rec_info(_gallobj->obj(_site_base)->crd(_epoch), _vBanc(4), _vBanc_base(4));
        }
        // use combmodel
        ...
        {
            if (_spdlog)
                SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + "Not enough processable observations!");
            t_gsppflt::_restore(QsavBP, XsavBP);
            return NO_MEAS;
        }
        ...
        if (_isBase)
        {
            ...
            {
                if (_spdlog)
                    SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + _site +
                                                    runEpoch.str_ymdhms(" combining DD observation failed at epoch: "));
                return NO_MEAS;
            }
        }
        Qsav = _Qx;
        ...
        if (_merge_pose(A) < 0)
        {
            if (_spdlog)
                SPDLOG_LOGGER_INFO(_spdlog,
                                   string("gintegration:  ") + _site + runEpoch.str_ymdhms(" merge failed at epoch: "));
            dx = 0.0;
            _Qx = Qsav;
            return NO_MEAS;
        }
        ...
        catch (...)
        {
            if (_spdlog)
                SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + _site +
                                                runEpoch.str_ymdhms(" meas_update failed at epoch: "));
            dx = 0.0;
            _Qx = Qsav;
            return NO_MEAS;
        }
...
int great::t_gintegration::_merge_pose(Matrix &A)
{
    try
    {
        ...
        A.SubMatrix(1, nobs, iattx, iattx + 2) =
            A.SubMatrix(1, nobs, icrdx, icrdx + 2) * Eigen2newMat(t_gbase::askew(sins.Ceb * lever));
        tmeas = _gnss_crt.sow() + _gnss_crt.dsec();
        if (fabs(sins.t - tmeas) > 0.5)
            throw("merge failed");
    }
    catch (...)
    {
        if (_spdlog)
            SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + "GNSS and INS infomation fusion failed!");
        //_reset();
        Flag = NO_MEAS;
        return -1;
    }
...
int great::t_gintegration::_merge_init()
{
    ...
    if (_shm.align)
    {
        sins.eth.Update(Cart2Geod(Eigen::Vector3d(gnss_pos[0], gnss_pos[1], gnss_pos[2]), false),
                        Eigen::Vector3d::Zero());
...
int great::t_gintegration::_GNSS_Update()
{
    ...
    else if (_ign_type == IGN_TYPE::TCI)
    {
        ...
        {
            if (_spdlog)
                SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") +
                                                _gnss_crt.str_ymdhms("TC GNSS/INS filter updating Failed at epoch: "));
        }
        else
        {
            if (_spdlog)
                SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") +
                                                _gnss_crt.str_ymdhms("TC GNSS/INS filter updating Succeed at epoch: "));
        }
    }
...
MEAS_TYPE great::t_gintegration::_getPOS(t_gposdata::data_pos &pos)
{
    ...
    if (pos.nSat < _shm.min_sat)
        res_type = NO_MEAS;
    if (_isBase && !pos.amb_state && !_shm._LCrtkFloatComb)
        res_type = NO_MEAS;
...
int great::t_gintegration::_getMeas()
{
    ...
    if (_Meas_Type.size())
    {
        ...
        if (_ign_type == TCI)
            _Qx = Eigen2BaseMatrix(_global_variance);
        Pk = _global_variance;
        Xk.conservativeResize(Pk.rows());
        Xk.block(nq, 0, size - nq, 1) = Eigen::VectorXd::Zero(size - nq);
        Phik = Eigen::MatrixXd::Identity(nq, nq);
...
bool great::t_gintegration::_time_valid(t_gtime gt, t_gtime inst)
{
    ...
    if ((abs(inst.diff(obsEpo)) < _shm.delay && inst >= obsEpo) || abs(inst.diff(obsEpo)) < 1e-6)
    {
        ...
        else
        {
            ...
            {
                SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + _site +
                                                gt.str_ymdhms(" no observation found at epoch: "));
            }
            res_valid = false;
        }
        if (_isBase)
        {
            ...
            else
            {
                if (_spdlog)
                {
                    SPDLOG_LOGGER_INFO(_spdlog, string("gintegration:  ") + _site_base +
                                                    gt.str_ymdhms(" no base observation found at epoch: "));
                }
                res_valid = false;
            }
        }
