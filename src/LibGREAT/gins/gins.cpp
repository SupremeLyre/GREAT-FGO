...
t_gsins::t_gsins(const t_gquat &qnb0, const Vector3d &vn0, const Vector3d &pos0, double t0)
{
    ...
    t = t0;
    eth.Update(pos0, vn0);
    Cnb = t_gbase::q2mat(qnb);
    att = t_gbase::q2att(qnb);
    Cbn = Cnb.transpose();
    vb = Cbn * vn;
    Kg = Ka = Vector3d::Ones();
...
great::t_gsins::t_gsins(gnut::t_gsetbase *set)
{
    ...
    qvb = t_gbase::a2qua(installation_att);
    Cvb = t_gbase::q2mat(qvb);
    Cbv = Cvb.transpose();
    pure_ins_time = 1.0;
}
...
void t_gsins::Update(const vector<Vector3d> &wm, const vector<Vector3d> &vm, const t_scheme &scm)
{
    ...
    imu.phim = Kg.asDiagonal() * imu.phim - eb * nts;
    imu.dvbm = Ka.asDiagonal() * imu.dvbm - db * nts;
    ...
    pos = pos + eth.v2dp(vn + vn1, nts_2);
    vn = vn1;
    qnb = t_gbase::rv2q(-eth.wnin * nts) * qnb * t_gbase::rv2q(imu.phim);
    Cnb = t_gbase::q2mat(qnb);
    att = t_gbase::m2att(Cnb);
    Cbn = Cnb.transpose();
    vb = Cbn * vn;
    eth.Update(pos, vn);
    pos_ecef = Geod2Cart(pos, false);
    Ceb = eth.Cen * Cnb;
    Cbe = Ceb.transpose();
    qeb = t_gbase::m2qua(Ceb);
    ve = eth.Cen * vn;
    ae = eth.Cen * an;
    pure_ins_time += nts;
}
...
void great::t_gsins::prt_header(ostringstream &os, bool imu_scale, bool use_odo)
{
    ...
    os << setw(18) << "X-ECEF" << setw(18) << "Y-ECEF" << setw(18) << "Z-ECEF";
    os << setw(10) << "VX" << setw(10) << "VY" << setw(10) << "VZ";
    os << setw(10) << "Pitch" << setw(10) << "Roll" << setw(10) << "Yaw";
    os << setw(12) << "GyroBiasX" << setw(12) << "GyroBiasY" << setw(12) << "GyroBiasZ" << setw(12) << "AcceBiasX"
       << setw(12) << "AcceBiasY" << setw(12) << "AcceBiasZ";
    if (imu_scale)
        os << setw(12) << "GyroScaleX" << setw(12) << "GyroScaleY" << setw(12) << "GyroScaleZ" << setw(12)
           << "AcceScaleX" << setw(12) << "AcceScaleY" << setw(12) << "AcceScaleZ";
    if (use_odo)
        os << setw(12) << "OdoScale";
    ...
    os << "# ";
    os << setw(15) << "(s)";
    os << setw(18) << "(m)" << setw(18) << "(m)" << setw(18) << "(m)";
    os << setw(10) << "(m/s)" << setw(10) << "(m/s)" << setw(10) << "(m/s)";
    os << setw(10) << "(deg)" << setw(10) << "(deg)" << setw(10) << "(deg)";
    os << setw(12) << "(deg/h)";
    os << setw(12) << "(deg/h)";
    os << setw(12) << "(deg/h)";
    os << setw(12) << "(mg)";
    ...
    if (imu_scale)
        os << setw(12) << "#" << setw(12) << "#" << setw(12) << "#" << setw(12) << "#" << setw(12) << "#" << setw(12)
           << "#";
    if (use_odo)
        os << setw(12) << "#";
...
void great::t_gsins::prt_sins(ostringstream &os)
{
    ...
    if (abs(xyz_out[0]) < 0.01 || content[0] == '0')
    {
        ...
        pos_out = xyz_out;
    string os_content = os.str();

    if (!os_content.empty() && (os_content[0] == '0' || os_content[0] == '1'))
    {
        os.str(os_content.substr(1));
    }
    ...
    os << fixed << setprecision(3) << setw(18) << pos_out(0) << setw(18) << pos_out(1) << setw(18) << pos_out(2);
    os << fixed << setprecision(3) << setw(10) << vel_out(0) << setw(10) << vel_out(1) << setw(10) << vel_out(2);
...
great::t_gsinskf::t_gsinskf(gnut::t_gsetbase *gset, t_spdlog spdlog, string site)
{
    ...
    _first_align = true;
    if (_shm._imu_scale)
        nq += 6; // gyro and acce scale factors
    if (_shm._imu_inst_rot)
        nq += 3; // gyro and acce scale factors
    if (_shm._odo)
        nq += 1; // default(in xml): 15, if(_odo)nq++;
...
void great::t_gsinskf::init_par(string site)
{
    ...
    if (_shm._imu_inst_rot)
    {
        ...
        param_of_sins.addParam(
            t_gpar(site, par_type::IMU_INST_ATT_Y, (int)par_type::IMU_INST_ATT_Y, "")); // un-observable by NHC
...
bool great::t_gsinskf::align_coarse(const vector<Eigen::Vector3d> &wm, const vector<Eigen::Vector3d> &vm,
                                    const t_scheme &scm)
{
    sins.t = scm.t;
    ...
    if (abs(_align_count - scm.align_time * scm.freq) < 1e-5)
    {
        ...
        wmm = wmm / _align_count;
        vmm = vmm / _align_count;
...
{
    Eigen::Vector3d vartheta =
        sins.Ceb * t_gbase::askew(lever) * sins.wib + t_gbase::askew(sins.eth.weie) * sins.Ceb * lever;
    Eigen::Matrix3d PhiH;
    int iodo = param_of_sins.getParam(_name, par_type::ODO_k, "");
    ...
    case NHC_MEAS:
        ...
        // }
        PhiH =
            sins.Cbe * t_gbase::askew(sins.ve) + t_gbase::askew(odo_lever) * sins.Cbe * t_gbase::askew(sins.eth.weie);
        ...
        {
            Hk.block(0, irot, 1, 3) =
                t_gbase::askew(sins.Cvb * (sins.vb + t_gbase::askew(sins.web) * odo_lever)).block(0, 0, 1, 3);
            Hk.block(1, irot, 1, 3) =
                t_gbase::askew(sins.Cvb * (sins.vb + t_gbase::askew(sins.web) * odo_lever)).block(2, 0, 1, 3);
...
bool great::t_gsinskf::_ins_init()
{
    ...
    kftk = sins.t;
    MeasVel = MeasPos = Eigen::Vector3d::Zero(), Flag = NO_MEAS;
    ...
    if (_spdlogkf)
    {
        SPDLOG_LOGGER_INFO(_spdlogkf, "initial misalignment std(deg): " + dbl2str(angle_tmp(0)) +
                                          dbl2str(angle_tmp(1)) + dbl2str(angle_tmp(2)));
        SPDLOG_LOGGER_INFO(_spdlogkf,
                           "initial velocity std(m/s): " + dbl2str(v_tmp(0)) + dbl2str(v_tmp(1)) + dbl2str(v_tmp(2)));
        SPDLOG_LOGGER_INFO(_spdlogkf, "initial posotion std(m): " + dbl2str(pos_tmp(0)) + dbl2str(pos_tmp(1)) +
                                          dbl2str(pos_tmp(2)));
        SPDLOG_LOGGER_INFO(_spdlogkf, "initial gyro bias std(deg/h): " + dbl2str(eb_tmp(0)) + dbl2str(eb_tmp(1)) +
                                          dbl2str(eb_tmp(2)));
        SPDLOG_LOGGER_INFO(_spdlogkf, "initial acce bias std(mg): " + dbl2str(db_tmp(0)) + dbl2str(db_tmp(1)) +
                                          dbl2str(db_tmp(2)));
    }
    ...
    Qt.head<15>() << angle_tmp * t_gglv::dpsh, v_tmp * t_gglv::mgpsHz, pos_tmp * t_gglv::mpsh, eb_tmp * t_gglv::dphpsh,
        db_tmp * t_gglv::mgpsh;
    ...
    if (_spdlogkf)
    {
        SPDLOG_LOGGER_INFO(_spdlogkf, "misalignment psd(deg/sqrt(h)): " + dbl2str(angle_tmp(0)) +
                                          dbl2str(angle_tmp(1)) + dbl2str(angle_tmp(2)));
        SPDLOG_LOGGER_INFO(_spdlogkf,
        ...
        SPDLOG_LOGGER_INFO(_spdlogkf, "gyro bias psd(deg/h/sqrt(h)): " + dbl2str(eb_tmp(0)) + dbl2str(eb_tmp(1)) +
                                          dbl2str(eb_tmp(2)));
        SPDLOG_LOGGER_INFO(_spdlogkf, "acce bias psd(mg/sqrt(h)): " + dbl2str(db_tmp(0)) + dbl2str(db_tmp(1)) +
                                          dbl2str(db_tmp(2)));
    }
...
int great::t_gsinskf::_ODO_Update()
{
    ...
    // get velocity
    double vf = 0;
    if (!_ododata->load(_ins_crt, vf, _shm.delay_odo))
        return -1;
    set_meas_ODO(vf, crt);
    int irc = _meas_update();
    return irc;
...
int great::t_gsinskf::_meas_update()
{
    if (Flag == NO_MEAS)
        return -1;
    Eigen::VectorXd Pxz = Eigen::VectorXd::Zero(Pk.rows()), Kk = Eigen::VectorXd::Zero(Pk.rows()),
                    Hi = Xk = Eigen::VectorXd::Zero(Pk.rows());
    this->_set_meas();
    ...
    if (_spdlogkf)
        SPDLOG_LOGGER_INFO(_spdlogkf,
                           "gintegration: " +
                               _ins_crt.str_ymdhms("LC " + meas2str(Flag) + "/INS filter updating Succeed at epoch: "));
...
bool great::t_gsinskf::_set_meas()
{
    ...
    case ODO_MEAS: // MeasVf=vb(1)-odo_vel
        ...
        Rk(0, 0) = _Cov_MeasOdo;
        measflag = 1;
        // cout << tmeas << "  " << Zk(0) << " " << -MeasVf * (1 + _odoscale) + (t_gbase::askew(sins.web) *
        // odo_lever)(1) << " " << sins.vb(1) << endl;
        break;
    case NO_MEAS:
        measflag = 0;
        break;
...
bool great::t_gsinskf::_resize()
{
    ...
    Hk = Eigen::MatrixXd::Zero(nr, Pk.rows());
