...
class LibGREAT_LIBRARY_EXPORT t_gsins
{
  public:
     ...
     */
    explicit t_gsins(const t_gquat &qnb0 = t_gquat(), const Eigen::Vector3d &vn0 = Eigen::Vector3d::Zero(),
                     const Eigen::Vector3d &pos0 = Eigen::Vector3d::Zero(), double t0 = 0.0);
     ...
     * @note set parameter value by gset
     */
    void set_posvel(const Eigen::Vector3d &pos0 = Eigen::Vector3d::Zero(),
                    const Eigen::Vector3d &vn0 = Eigen::Vector3d::Zero());
  ...
  public:
    ...
    Eigen::Vector3d _tauG, _tauA, _tauGScale, _tauAScale; /// Markov time
    Eigen::Matrix3d Cnb, Cbn;                             /// DCM,Scale
    t_gquat qnb;                                          /// quat

    Eigen::Vector3d ve, ae, fe, pos_ecef, xyz_out;
...
class LibGREAT_LIBRARY_EXPORT t_gsinskf
{
  public:
     ...
     */
    virtual bool align_coarse(const vector<Eigen::Vector3d> &wm, const vector<Eigen::Vector3d> &vm,
                              const t_scheme &scm);
    ...
    /**
     * @brief print Pk into otringstream
     *
     * @param[in] t				time
     * @param[in] Pk				vector of towrite
     * @param[out] os			ostringstream
     *
     */
    /**
     * @brief Initial Alignment.
