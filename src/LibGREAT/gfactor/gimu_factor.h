#ifndef GIMU_FACTOR_H
#define GIMU_FACTOR_H
/**
 * @file         gimu_factor.h
...
#include "gintegration_base.h"

namespace gfgo
{
/**
 *@brief IMUFactor Class for constructing pre-integration residual block
 */
class LibGREAT_LIBRARY_EXPORT IMUFactor : public ceres::SizedCostFunction<15, 7, 9, 7, 9>
{
  public:
    IMUFactor() = delete;
    ...
    virtual bool Evaluate(double const *const *parameters, double *residuals, double **jacobians) const
    {
        ...
        if (jacobians)
        {
            ...
            if (jacobians[0])
            {
                ...
                jacobian_pose_i.block<3, 3>(O_P, O_R) = t_gfgo_utility::skewSymmetric(
                    Qi.inverse() * (0.5 * pre_integration->gravity * sum_dt * sum_dt + Pj - Pi - Vi * sum_dt));

#if 0
					jacobian_pose_i.block<3, 3>(O_R, O_R) = -(Qj.inverse() * Qi).toRotationMatrix();
#else
                Eigen::Quaterniond corrected_delta_q =
                    pre_integration->delta_q * t_gfgo_utility::deltaQ(dq_dbg * (Bgi - pre_integration->linearized_bg));
                jacobian_pose_i.block<3, 3>(O_R, O_R) =
                    -(t_gfgo_utility::Qleft(Qj.inverse() * Qi) * t_gfgo_utility::Qright(corrected_delta_q))
                         .bottomRightCorner<3, 3>();
#endif

                jacobian_pose_i.block<3, 3>(O_V, O_R) =
                    t_gfgo_utility::skewSymmetric(Qi.inverse() * (pre_integration->gravity * sum_dt + Vj - Vi));
            ...
            if (jacobians[1])
            {
                ...
                jacobian_speedbias_i.block<3, 3>(O_R, O_BG - O_V) =
                    -t_gfgo_utility::Qleft(Qj.inverse() * Qi * pre_integration->delta_q).bottomRightCorner<3, 3>() *
                    dq_dbg;
#endif
            ...
            if (jacobians[2])
            {
                ...
                jacobian_pose_j.block<3, 3>(O_R, O_R) =
                    t_gfgo_utility::Qleft(corrected_delta_q.inverse() * Qi.inverse() * Qj).bottomRightCorner<3, 3>();
#endif
