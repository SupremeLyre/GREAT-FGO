...
t_gambiguity::t_gambiguity(string site, t_gsetbase *gset)
    : _part_fix(false), _is_first(true), _is_first_wl(true), _is_first_nl(true), _is_first_ewl(true),
      _is_first_ewl24(true), _is_first_ewl25(true), _amb_fixed(false), _obstype(OBSCOMBIN::IONO_FREE),
      _upd_mode(UPD_MODE::UPD), _fix_mode(FIX_MODE::SEARCH)
{
    _site = site;
    _gset = gset;
...
int t_gambiguity::processBatch(const t_gtime &t, t_gflt *gflt, string mode)
{
    ...
    {
        if (t.diff(_last_fix_time[mode]["sum"]) >= _ctrl_last_fixepo_gap * _interval &&
            fixed_amb.size() < _ctrl_min_fixed_num)
            return -1;
    }
    ...
    if (_spdlog)
    {
        SPDLOG_LOGGER_INFO(_spdlog, "ratio: {}, outRatio: {}", _ratio, _outRatio);
    }
...
double t_gambiguity::lambdaSolve(double &ratio, const Matrix &anor, const ColumnVector &fltpar, ColumnVector &ibias,
                                 bool parlamb)
{
...
bool t_gambiguity::_checkAmbDepend(bool isFirst, int iNamb, int *iNdef, int iN_oneway, int *arriIpt2ow, int iMaxamb_ow,
        ...
        for (auto itsat2 = itsat1 + 1; itsat2 != InnerLoopEnd; itsat2++)
        {
            ...
            if (itsat1->prn.substr(0, 1) == "R")
            {
                if (_sys_wavelen[itsat1->prn]["NL"] == 0)
                    continue;
                dd.rlc = itsat1->pred / _sys_wavelen[itsat1->prn]["NL"] -
                         itsat2->pred / _sys_wavelen[itsat2->prn]["NL"]; // Unit:Cycle
                if (_obstype == OBSCOMBIN::RAW_ALL)
                {
                    dd.rwl_R1 =
                        itsat2->pred / _sys_wavelen[itsat2->prn]["NL"]; // temp save, it is not its real meaning!
                }
            }
...
bool t_gambiguity::_calDDAmbWL()
{
    ...
    for (auto itdd = _DD.begin(); itdd != _DD.end();)
    {
        ...
        if (itdd->ddSats.size() != 2 || _MW.find(sat1) == _MW.end() || _MW.find(sat2) == _MW.end() ||
            _MW[sat1][1] <= 2.0 || _MW[sat2][1] <= 2.0)
        {
            itdd = _DD.erase(itdd);
            continue;
        }
    ...
    else
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                "Warning[t_gambiguity::_applySingleUpd] : Undefined UPD Mode (WL/NL) : " + mode);
        return false;
    }
    return true;
}
    ...
    for (auto itdd = _DD.begin(); itdd != _DD.end();)
    {
            ...
            if (_frequency >= 3 && _obstype == OBSCOMBIN::RAW_ALL)
            {
                     ...
                     !_getSingleUpd("EWL", _ewl_Upd_time, sat2, upd_ewl2, sig)))
                {
                    if (itdd->ambtype == "AMB_L3")
                    {
                        if (_spdlog)
                            SPDLOG_LOGGER_DEBUG(
                                _spdlog, "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : EWL, Sat: " + sat1 +
                                             " " + sat2);
                        itdd = _DD.erase(itdd);
                        continue;
                    }
            ...
            if (_frequency >= 4 && _obstype == OBSCOMBIN::RAW_ALL)
            {
                     ...
                     !_getSingleUpd("EWL24", _ewl24_Upd_time, sat2, upd_ewl24_2, sig)))
                {
                    if (itdd->ambtype == "AMB_L4")
                    {
                        if (_spdlog)
                            SPDLOG_LOGGER_DEBUG(
                                _spdlog, "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : EWL24, Sat: " +
                                             sat1 + " " + sat2);
                        itdd = _DD.erase(itdd);
                        continue;
                    }
            ...
            if (_frequency == 5 && _obstype == OBSCOMBIN::RAW_ALL)
            {
                     ...
                     !_getSingleUpd("EWL25", _ewl25_Upd_time, sat2, upd_ewl25_2, sig)))
                {
                    if (itdd->ambtype == "AMB_L5")
                    {
                        if (_spdlog)
                            SPDLOG_LOGGER_DEBUG(
                                _spdlog, "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : EWL25, Sat: " +
                                             sat1 + " " + sat2);
                        itdd = _DD.erase(itdd);
                        continue;
                    }
            ...
            if (_upd_mode == UPD_MODE::UPD)
            {
                    ...
                    !_getSingleUpd("WL", _wl_Upd_time, sat2, upd_wl2, sig))
                {
                    if (_spdlog)
                        SPDLOG_LOGGER_DEBUG(
                            _spdlog,
                            "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : WL, Sat: " + sat1 + " " + sat2);
                    itdd = _DD.erase(itdd);
                    continue;
                }
            }
            ...
            if (_upd_mode == UPD_MODE::UPD)
            {
                if (!_getSingleUpd("NL", t, sat1, upd_nl1, sig) || !_getSingleUpd("NL", t, sat2, upd_nl2, sig))
                {
                    if (_spdlog)
                        SPDLOG_LOGGER_DEBUG(
                            _spdlog,
                            "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : NL, Sat: " + sat1 + " " + sat2);
                    itdd = _DD.erase(itdd);
                    continue;
                }
            }
        ...
        if (_obstype == OBSCOMBIN::IONO_FREE)
        {
            if (sat1.substr(0, 1) != "R")
            {
                ...
                itdd->rnl =
                    itdd->rlc / _sys_wavelen[sat1.substr(0, 1)]["NL"] -
        ...
        if (_gupd)
        {
            ...
            if (_frequency >= 3 && mode == "EWL")
            {
                    ...
                    (!_getSingleUpd("EWL", _ewl_Upd_time, sat2, upd_ewl2, sig) && mode == "EWL"))
                {
                    if (_spdlog)
                        SPDLOG_LOGGER_DEBUG(
                            _spdlog,
                            "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : EWL, Sat: " + sat1 + " " + sat2);
                    itdd = _DD.erase(itdd);
                    continue;
                }
            }
            else if (_frequency >= 4 && mode == "EWL24")
            {
                    ...
                    (!_getSingleUpd("EWL24", _ewl24_Upd_time, sat2, upd_ewl2, sig) && mode == "EWL24"))
                {
                    if (_spdlog)
                        SPDLOG_LOGGER_DEBUG(
                            _spdlog, "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : EWL24, Sat: " + sat1 +
                                         " " + sat2);
                    itdd = _DD.erase(itdd);
                    continue;
                }
            ...
            else if (_frequency == 5 && mode == "EWL25")
            {
                    ...
                    (!_getSingleUpd("EWL25", _ewl25_Upd_time, sat2, upd_ewl2, sig) && mode == "EWL25"))
                {
                    if (_spdlog)
                        SPDLOG_LOGGER_DEBUG(
                            _spdlog, "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : EWL25, Sat: " + sat1 +
                                         " " + sat2);
                    itdd = _DD.erase(itdd);
                    continue;
                }
            ...
            {
                if (_spdlog)
                    SPDLOG_LOGGER_DEBUG(_spdlog, "Warning[t_gambiguity::_applyUpd] : _getSingleUpd Wrong : WL, Sat: " +
                                                     sat1 + " " + sat2);
                itdd = _DD.erase(itdd);
                continue;
            }
...
bool t_gambiguity::_fixAmbIF()
{
    ...
    for (auto itdd = _DD.begin(); itdd != _DD.end();)
    {
        ...
        {
            bdeci.bdeci(itdd->rwl_R1, itdd->srwl_R1, 1, _map_WL_decision["maxdev"], _map_WL_decision["maxsig"], prob,
                        alpha1);
            bdeci.bdeci(itdd->rwl_R2, itdd->srwl_R2, 1, _map_WL_decision["maxdev"], _map_WL_decision["maxsig"], prob,
                        alpha2);
            if (alpha1 > _map_WL_decision["alpha"] && alpha2 > _map_WL_decision["alpha"])
            {
                itdd->isWlFixed = true;
            }
            if (_part_fix && abs(itdd->rwl_R1 - round(itdd->rwl_R1)) < 0.25 &&
                abs(itdd->rwl_R2 - round(itdd->rwl_R2)) < 0.25)
            {
                itdd->isWlFixed = true;
            }
        }
...
bool t_gambiguity::_fixAmbUDUC()
{
    ...
    for (auto itdd = _DD.begin(); itdd != _DD.end(); itdd++)
    {
        ...
        if (_fix_epo_num["NL"][sat1] > 20 && _fix_epo_num["NL"][sat2] > 20 && _lock_epo_num[sat1] > 200 &&
        ...
        if (mode == "WL")
        {
                ...
                _lock_epo_num[sat2] > 200)
            {
                if (abs(itdd->rwl - round(itdd->rwl)) < _map_WL_decision["maxdev"] * 1.1)
                {
                    ...
                    itdd->iwl = round(itdd->rwl);
                    if (_spdlog)
                        SPDLOG_LOGGER_INFO(_spdlog, "WL Fixed by UDUC, Sat: " + sat1 + " " + sat2 +
                                                        ", Site: " + itdd->site + ", WL: " + to_string(itdd->iwl));
                }
            }
        }
        else if (mode == "EWL")
...
int t_gambiguity::_selectAmb(int korder, int namb)
{
    ...
    for (auto itdd = _DD.begin(); itdd != _DD.end();)
    {
        bool wl_fix = (_obstype == gnut::OBSCOMBIN::RAW_MIX && itdd->isSngleFreq != true ||
                       _obstype != gnut::OBSCOMBIN::RAW_MIX && _obstype != gnut::OBSCOMBIN::RAW_SINGLE);
...
bool t_gambiguity::_prepareCovariance(t_gamb_cmn *amb_cmn, SymmetricMatrix &covariance, vector<double> &value)
{
    ...
    for (auto itdd1 = _DD.begin(); itdd1 != _DD.end(); itdd1++)
    {
        ...
        for (auto itdd2 = itdd1; itdd2 != _DD.end(); itdd2++)
        {
            ...
            covariance(row, col) =
                (Q(1, 1) - Q(2, 1) - Q(1, 2) + Q(2, 2)) / itdd1->factor / itdd2->factor; // unit [cycle]
        }
    }
    // Unit weight
    ...
    if (value.size() == 0 || covariance.size() == 0)
    {
        if (_spdlog)
            SPDLOG_LOGGER_ERROR(
                _spdlog, "ERROR[t_gambiguity::_prepareCovariance] : prepare Double-Difference covariance is Wrong");
        return false;
    }
    else
        return true;
...
bool t_gambiguity::_prepareCovarianceWL(t_gamb_cmn *amb_cmn, SymmetricMatrix &covariance, vector<double> &value,
                                        string mode)
{
    int row = 0, col = 0;
    ...
    for (auto itdd1 = _DD.begin(); itdd1 != _DD.end(); itdd1++)
    {
        ...
        for (auto itdd2 = itdd1; itdd2 != _DD.end(); itdd2++)
        {
            ...
            // Column of covariance-matrix
            col = distance(_DD.begin(), itdd2) + 1;
            op_dd << 1 / (lambda_1 * lambda_1) << -1 / (lambda_1 * lambda_2) << -1 / (lambda_1 * lambda_2)
                  << 1 / (lambda_2 * lambda_2);
            Matrix Q(2, 2);
...
bool t_gambiguity::_ambSolve(t_gamb_cmn *amb_cmn, vector<int> &fixed_amb, string mode)
{
    ...
    if (_DD.size() < _full_fix_num)
    {
        ...
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog,
                                "Warning[t_gambiguity::_ambSolve] : too few candidate ambiguities for LAMBDA Search");
        return false;
    }
        ...
        if (newamb < 2)
        {
            for (unsigned int i = 1; i <= value.size(); i++)
            {
                ...
                if (_fix_epo_num[mode][get<0>(itdd->ddSats[0])] == 0 ||
                    _fix_epo_num[mode][get<0>(itdd->ddSats[1])] == 0 || _lock_epo_num[get<0>(itdd->ddSats[0])] < 5 ||
    ...
    if (mode == "WL")
    {
        ...
        else
        {
            if (_spdlog)
                SPDLOG_LOGGER_DEBUG(
                    _spdlog, "Warning[t_gambiguity::_ambSolve] : LAMBDA Search can't get candidate fixed ambiguities");
            return false;
        }
    }
    else if (mode == "EWL")
...
bool t_gambiguity::_addFixConstraint(t_gflt *gflt)
{
    ...
    {
        bool wl_fix = ((_obstype == gnut::OBSCOMBIN::RAW_MIX && itdd->isSngleFreq != true) ||
                       _obstype != gnut::OBSCOMBIN::RAW_MIX);
        ...
        if (_obstype == OBSCOMBIN::IONO_FREE)
        {
            ...
            if (sys != "R")
            {
                integer = (itdd->inl - itdd->sd_rnl_cor +
                           round(itdd->rwl) * _sys_wavelen[sys]["WL"] / _sys_wavelen[sys]["L2"]) *
                          itdd->factor;
            }
            else
            {
...
bool t_gambiguity::_addFixConstraintWL(t_gflt *gflt, string mode)
{
    ...
    for (auto itdd = _DD.begin(); itdd != _DD.end(); itdd++)
    {
        ...
        virtual_equ.add_equ(B, p0, dl, _site,
                            get<0>(itdd->ddSats[0]) + "_" + get<0>(itdd->ddSats[2]) + "_" + get<0>(itdd->ddSats[1]) +
                                "_" + get<0>(itdd->ddSats[3]),
                            type, false);
        virtual_equ.chageNewMat(B_mat, P_mat, l_mat, gflt->npar_number());
...
void t_gambiguity::_writeBoot(double BootStrapping)
{
    ...
    os << " " << setw(4) << nepoch << "     " << fixed << setw(5) << setprecision(2) << BootStrapping * 100.0 << "%"
       << endl;
