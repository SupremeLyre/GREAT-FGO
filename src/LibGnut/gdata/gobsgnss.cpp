...
t_gobsgnss::t_gobsgnss()
    : t_gdata(), _apr_ele(-1), _channel(DEF_CHANNEL), _rtcm_end(2), _health(true), _dcb_correct_mark(false),
      _bds_code_bias_mark(false), _range_smooth_mark(false)
{
...
t_gobsgnss::t_gobsgnss(t_spdlog spdlog)
    : t_gdata(spdlog), _apr_ele(-1), _channel(DEF_CHANNEL), _rtcm_end(2), _health(true), _dcb_correct_mark(false),
      _bds_code_bias_mark(false), _range_smooth_mark(false)
{
    id_type(t_gdata::OBSGNSS);
    id_group(t_gdata::GRP_OBSERV);
...
t_gobsgnss::t_gobsgnss(t_spdlog spdlog, const string &sat)
    : t_gdata(spdlog), _apr_ele(-1), _channel(DEF_CHANNEL), _rtcm_end(2), _health(true), _dcb_correct_mark(false),
      _bds_code_bias_mark(false), _range_smooth_mark(false), _satid(sat), _gsys(t_gsys::char2gsys(sat[0]))
{
    id_type(t_gdata::OBSGNSS);
    id_group(t_gdata::GRP_OBSERV);
}

t_gobsgnss::t_gobsgnss(t_spdlog spdlog, const string &site, const string &sat, const t_gtime &t)
    : t_gdata(spdlog), _staid(site), _satid(sat), _gsys(t_gsys::char2gsys(_satid[0])), _epoch(t), _apr_ele(-1),
      _channel(DEF_CHANNEL), _rtcm_end(2), _health(true), _dcb_correct_mark(false), _bds_code_bias_mark(false),
...
bool t_gobsgnss::apply_bias(t_gallbias *allbias)
{
    ...
    if (_spdlog)
    {
        SPDLOG_LOGGER_WARN(_spdlog, "{} : No OSB bias found, try to apply DCB", this->site());
    }
...
bool t_gobsgnss::apply_osb(t_gallbias *allbias)
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "{} : NO OSB FILE", this->site());
        return true;
    }
...
double t_gobsgnss::interFreqDcb(t_gallbias &allbias, const GOBSBAND &band1, const GOBSBAND &band2) const
{
    ...
    {
        if (_spdlog)
            SPDLOG_LOGGER_DEBUG(_spdlog, "interFreqDcb {}",
                                gobs2str(obs1) + " " + gobs2str(obs2) + " " + " " + gsat + " " + " " + grec + " " +
...
#ifdef DEBUG
    cout << _epoch.str_hms() << " sat: " << sat() << " band: " << gobsband2str(gb.band())
         << " attr: " << gobsattr2str(gb.attr()) << endl;
#endif
...
double t_gobsgnss::frequency_lc(const int &band1, const double &coef1, const int &band2, const double &coef2,
...
double t_gobsgnss::pnf_lc(const int &band1, const double &coef1, const int &band2, const double &coef2,
                          const int &band3, const double &coef3) const
{
    double f1 = 1.0;
    double f2 = 1.0;
...
double t_gobsgnss::_lcf_range(const t_gobs *gobs1, const int &coef1, const t_gobs *gobs2, const int &coef2,
...
double t_gobsgnss::_lcf_phase(const t_gobs *gobs1, const int &coef1, const t_gobs *gobs2, const int &coef2,
                              const t_gobs *gobs3, const int &coef3) const
{
...
int t_gobsgnss::_coef_multpath(const GOBSBAND &bC, double &cC, const GOBSBAND &b1, double &c1, const GOBSBAND &b2,
                               double &c2) const
{
    if (b1 == b2)
    {
...
double t_gobsgnss::EWL_cycle(const t_gobs &gL1, const t_gobs &gL2, const t_gobs &gL3, const t_gobs &gC1,
                             const t_gobs &gC2) const
{
#ifdef BMUTEX
    boost::mutex::scoped_lock lock(_mutex);
#endif
...
double t_gobsgnss::_lc_range(const t_gobs *g1, const double &coef1, const t_gobs *g2, const double &coef2,
...
double t_gobsgnss::_lc_phase(const t_gobs *g1, const double &coef1, const t_gobs *g2, const double &coef2,
                             const t_gobs *g3, const double &coef3) const
{
    double L1 = NULL_GOBS;
    double L2 = NULL_GOBS;
...
{
    return (this->first.type() < t.first.type() && this->first.band() < t.first.band() &&
