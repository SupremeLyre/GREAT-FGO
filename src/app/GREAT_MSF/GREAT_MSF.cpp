...
int main(int argc, char **argv)
{
    ...
    t_gododata *godo = new t_gododata();
    godo->spdlog(my_logger);
    ...
    for (size_t i = 0; i < inp.size() && itINP != inp.end(); ++i, ++itINP)
    {
        ...
        else if (ifmt == IFMT::ODO_INP)
        {
            gdata = godo;
            tgcoder = new t_odofile(&gset, "", 40960);
        }
        else if (ifmt == IFMT::SP3_INP)
        {
            gdata = gorb;
            tgcoder = new t_sp3(&gset, "", 8172);
        }
        else if (ifmt == IFMT::RINEXO_INP)
        {
            gdata = gobs;
            tgcoder = new t_rinexo(&gset, "", 4096);
        }
        else if (ifmt == IFMT::RINEXC_INP)
        {
            gdata = gorb;
            tgcoder = new t_rinexc(&gset, "", 4096);
        }
        else if (ifmt == IFMT::RINEXN_INP)
        {
            gdata = gorb;
        ...
        else if (ifmt == IFMT::BLQ_INP)
        {
            gdata = gotl;
            tgcoder = new t_blq(&gset, "", 4096);
        }
        else if (ifmt == IFMT::BIASINEX_INP)
        {
            gdata = gbia;
            tgcoder = new t_biasinex(&gset, "", 20480);
        }
        else if (ifmt == IFMT::BIAS_INP)
        {
            gdata = gbia;
            tgcoder = new t_biabernese(&gset, "", 20480);
        }
        else if (ifmt == IFMT::DE_INP)
        {
            gdata = gde;
            tgcoder = new t_dvpteph405(&gset, "", 4096);
        }
        else if (ifmt == IFMT::EOP_INP)
        {
            gdata = gerp;
            tgcoder = new t_poleut1(&gset, "", 4096);
        }
        else if (ifmt == IFMT::IFCB_INP)
        {
            gdata = gifcb;
        ...
        if (tgcoder)
        {
            ...
            if (ifmt != IFMT::IMU_INP && ifmt != IFMT::ODO_INP)
            ...
            SPDLOG_LOGGER_INFO(my_logger, std::string("main:  ") + "READ: " + path +
                                              " time: " + dbl2str(lstepoch.diff(runepoch)) + " sec");
            // Delete
            delete tgio;
            delete tgcoder;
        }
    ...
    while (i < nsite)
    {
        ...
        if (isBase)
        {
            ...
            {
                SPDLOG_LOGGER_INFO(my_logger, std::string("main:  ") + std::string("main:  ") +
                                                  "No two site/data for processing!");
                i++;
                continue;
            }
        }
        ...
        lstepoch = t_gtime::current_time(t_gtime::GPS);
        // Write the log file
        SPDLOG_LOGGER_INFO(my_logger, std::string("main:  ") + site_base + site +
                                          "MSF processing finished : duration  " + dbl2str(lstepoch.diff(runepoch)) +
                                          " sec");
    ...
    for (unsigned int i = 0; i < vgmsf.size(); ++i)
    {
        if (vgmsf[i])
            delete vgmsf[i];
    }
